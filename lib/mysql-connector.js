...
const ERROR_DESCRIPTIONS = {
  ...
  ER_TABLEACCESS_DENIED_ERROR: 'Table access denied. The user lacks required table-level privileges.',
  ER_NO_SUCH_TABLE: 'Table does not exist and could not be created. Check user CREATE privileges.',
  ER_LOCK_DEADLOCK: 'Transaction deadlock detected. Retry the operation.',
  ER_LOCK_WAIT_TIMEOUT: 'Lock wait timeout exceeded. The database may be under heavy load.',
  ER_CON_COUNT_ERROR: 'Too many connections to the MySQL server. Try again later or increase max_connections.',
  ER_HOST_NOT_PRIVILEGED: 'This host is not allowed to connect. Check MySQL user host grants.',
  EPIPE: 'Broken pipe. The connection was unexpectedly closed during communication.',
  EAI_AGAIN: 'DNS lookup timed out. Check network connectivity and DNS configuration.'
};

/**
 * Detect whether an error message looks like it came from an HTTP/API proxy
 * rather than the MySQL protocol.
 *
 * @param {Error} err
 * @returns {{ isApiError: boolean, statusCode: number|null }}
 */
function detectApiProxyError(err) {
  const msg = (err.sqlMessage || err.message || '').toLowerCase();
  const apiPatterns = /(?:api\s+(?:failed|error)|internal\s+server\s+error|bad\s+gateway|service\s+unavailable|gateway\s+timeout)/;
  const statusMatch = (err.sqlMessage || err.message || '').match(/\((\d{3})\)/);
  const statusCode = statusMatch ? parseInt(statusMatch[1], 10) : null;
  const isApiError = apiPatterns.test(msg) || (statusCode !== null && statusCode >= 400);
  return { isApiError, statusCode };
}
...
async function testConnection(config) {
  ...
  try {
    ...
    return {
      success: false,
      message: description || `MySQL connection failed: ${fallbackMessage}`,
      error: {
