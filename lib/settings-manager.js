...
const { DEFAULT_OPTIONS, ERROR_DESCRIPTIONS, detectApiProxyError } = require('./mysql-connector');
...
async function saveSettings(creds, records) {
  ...
  try {
    ...
    // Pre-flight check: verify the connection is usable before running operations
    try {
      await connection.execute('SELECT 1');
    } catch (pingErr) {
      const apiCheck = detectApiProxyError(pingErr);
      if (apiCheck.isApiError) {
        return {
          success: false,
          message: 'The database host returned an API/proxy error instead of a MySQL protocol response. ' +
            'Verify that the host and port point to a MySQL server (not an HTTP API endpoint).',
          error: {
            code: pingErr.code || 'API_PROXY_ERROR',
            errno: pingErr.errno || null,
            sqlState: pingErr.sqlState || null,
            sqlMessage: pingErr.sqlMessage || pingErr.message || 'Database API error',
            description: 'The connection target appears to be an HTTP/API gateway, not a MySQL server.',
            apiStatusCode: apiCheck.statusCode,
            fatal: true
          },
          latencyMs: Date.now() - startTime,
          troubleshooting: [
            'Verify the host and port point directly to a MySQL server.',
            'If using a cloud database, ensure you are using the MySQL protocol endpoint (not a REST/HTTP API).',
            'Check that no HTTP proxy or API gateway is intercepting the database connection.',
            'Contact your database hosting provider if the issue persists.'
          ]
        };
      }
      throw pingErr;
    ...
    for (const record of records) {
      ...
      await connection.execute(upsertSQL, [record.key, value, value]);
    ...
    const apiCheck = detectApiProxyError(err);
    ...
    if (apiCheck.isApiError) {
      ...
      message = description || `Failed to save settings: ${fallbackMessage}`;
    }
    ...
    return {
      ...
      error: {
        ...
        ...(apiCheck.isApiError && { apiStatusCode: apiCheck.statusCode })
      },
      latencyMs: elapsed,
      troubleshooting: apiCheck.isApiError
        ? [
            'Verify the host and port point directly to a MySQL server.',
            'If using a cloud database, ensure you are using the MySQL protocol endpoint (not a REST/HTTP API).',
            'Check that no HTTP proxy or API gateway is intercepting the database connection.',
