...
app.post('/api/save-settings', async (req, res) => {
  ...
  try {
    ...
    const result = await withRetry(
      async () => {
        ...
        try {
          ...
          return {
            success: false,
            message: `Failed to save settings: ${fallback}`,
            error: {
        ...
        if (!saveResult.success && saveResult.error) {
          const retryableCodes = ['ECONNRESET', 'ETIMEDOUT', 'PROTOCOL_CONNECTION_LOST', 'EPIPE', 'ECONNREFUSED', 'EAI_AGAIN', 'ER_LOCK_DEADLOCK'];
      ...
      (attempt, delay, err) => {
        try {
          ...
          // Prevent logging failures from aborting retries
        }
      }
    );
    ...
    return res.status(400).json({
      success: false,
      message: `Failed to save settings: ${fallbackMessage}`,
      error: {
        code: err.code || 'INTERNAL_ERROR',
...
app.post('/api/get-settings', async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({
      success: false,
      message: `Failed to retrieve settings from MySQL: ${fallbackMessage}`,
      error: {
        code: err.code || 'INTERNAL_ERROR',
        sqlMessage: fallbackMessage
      },
...
app.post('/api/delete-settings', async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({
      success: false,
      message: `Failed to delete settings from MySQL: ${fallbackMessage}`,
      error: {
        code: err.code || 'INTERNAL_ERROR',
        sqlMessage: fallbackMessage
      },
